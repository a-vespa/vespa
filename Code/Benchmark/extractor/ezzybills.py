...
def get_extraction_details(filepath, file):
    try:
        ...
        while True:
            r4 = requests.get(url + '/workflowStatus?invoiceid=' +
                              invoiceID, cookies=r.cookies, params=api_key, verify=False)
        ...
        # Get invoice total
        r5 = requests.get(url + '/getInvoiceHeaderBlocks?invoiceid=' +
                          invoiceID, cookies=r.cookies, params=api_key, verify=False)
