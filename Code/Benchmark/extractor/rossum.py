...
def get_extraction_details(filepath, file):
    try:
        ...
        response = requests.request(
            'POST', url, headers=headers, data=payload, files=files, allow_redirects=False, verify=False)
        ...
        while is_reviewed == False:
            ...
            response1 = requests.get(
                statusurl, headers=headers, allow_redirects=False, verify=False)
            status_json = json.loads(response1.text)
            if status_json["status"] == "to_review":
                extraction_time = time.time() - start_time
                is_reviewed = True
                extraction_details_url = 'https://api.elis.rossum.ai/v1/queues/34281/export?status=to_review&format=json&id=' + \
                    str(status_json["id"])
                extraction_details = requests.get(
                    extraction_details_url, headers=headers, allow_redirects=False, verify=False)
